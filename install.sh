...
download_atomic_anim() {
  ...
  # Phase 3: main download bar — target ~35 sec total
  # 100 steps, ~0.35s each but varied
  printf "${CY}  %-18s${RST}  " "atomic.py"
  local W=36
  local total_steps=100
  for (( i=1; i<=total_steps; i++ )); do
    local filled=$(( i * W / total_steps ))
    local empty=$(( W - filled ))
    local pct=$i

    # Glitch the bar occasionally
    if (( RANDOM % 12 == 0 && i < 90 )); then
      local fake_pct=$(( pct - RANDOM % 8 ))
      (( fake_pct < 0 )) && fake_pct=0
      local fake_filled=$(( fake_pct * W / total_steps ))
      local fake_empty=$(( W - fake_filled ))
      local B1f B2f
      B1f=$(python3 -c "print('█'*$fake_filled, end='')")
      B2f=$(python3 -c "print('░'*$fake_empty, end='')")
      printf "\r${CY}  %-18s${RST}  ${RD}%s${DM}%s${RST}  ${YL}%3d%%${RST}" \
        "atomic.py" "$B1f" "$B2f" "$fake_pct"
      sleep 0.09
    fi

    local B1 B2
    B1=$(python3 -c "print('█'*$filled, end='')")
    B2=$(python3 -c "print('░'*$empty, end='')")
    printf "\r${CY}  %-18s${RST}  ${G}%s${DM}%s${RST}  ${WH}%3d%%${RST}" \
      "atomic.py" "$B1" "$B2" "$pct"

    # Vary speed: slow at start, bursts in middle, slight stall near end
    if (( i < 15 )); then
      sleep 0.28
    elif (( i < 40 )); then
      sleep $(python3 -c "import random; print(round(random.uniform(0.25,0.45), 2))")
    elif (( i < 70 )); then
      sleep $(python3 -c "import random; print(round(random.uniform(0.28,0.42), 2))")
    elif (( i < 88 )); then
      sleep $(python3 -c "import random; print(round(random.uniform(0.22,0.38), 2))")
    elif (( i < 95 )); then
      sleep 0.55   # dramatic stall near end
    else
      sleep 0.20
    fi
  done
...
install_tools() {
  ...
  for tool in "${tools[@]}"; do
    local steps=$(( 16 + RANDOM % 12 ))
    local delay
    delay=$(python3 -c "import random; print(round(random.uniform(0.04,0.10),3))")
    draw_bar "$tool" "$steps" "$delay"
...
# ── MAIN ──────────────────────────────────────────────────────────────────────
main() {
  # Long glitch intro (30 frames)
  glitch_logo_anim 30 8
